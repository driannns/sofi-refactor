...
<div class="ml-2 mb-5">
    ...
    <span class="btn btn-outline-primary float-right mb-2" data-toggle="modal" data-target="#informasirevisi">Informasi Revisi</span>
    ...
    <tbody id="revisionPlace">
        ...
        <tr>
            ...
            <input type="text" hidden name='dokumen_id[]' value="{{$revision->dokumen_id}}" >
                @if($revisions != null)
                @if(!file_exists($revision->dokumen->file_url))
              
                        File tidak ada
                        @else
                <div class="form-group col-sm-3">
                    <button type="button" name="button" class="btn btn-outline-success" data-toggle="modal"
                        data-target="#viewDokumenTA{{$revision->id}}">Lihat {{$revision->dokumen->nama}}</button>
                </div>
...
<button type="button" id="addRevisi" class='btn btn-outline-primary mb-3' {{ $isLate ? 'disabled' : '' }}>
    Tambah Baris Revisi
</button>
...
@if($revisions != null)
@foreach($revisions as $revision)
<div class="modal fade" id="viewDokumenTA{{$revision->id}}" tabindex="-1" role="dialog" aria-labelledby="feedbackModal"
    aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    @if($revisions != null)
                    <embed src="/{{$revision->dokumen->file_url}}" type="application/pdf" frameborder="0" width="100%"
                        height="400px">
                    @endif
                </div>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#addRevisi').on('click', function () {
            ...
            "<tr>" +
                ...
                "<td>" +
                "<div class='form-group col-sm-12'>" +
                " <a href='javascript:attend()' class='btn btn-outline-primary w-100 fileuploadspan ' id='submit-button' >Upload</a>" +
  ...
  <script>
    ...
    function attend2(link) {
        ...
        Swal.fire({
            title: 'Perhatian!',
            text: 'Jika File Sudah DI SimpanTidak Dapat Di Ubah',
            icon: 'info',
