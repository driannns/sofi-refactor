...
class tidakLulusByMahasiswa extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:tidakLulusByMahasiswa';
    ...
    {
        try{
            //reset to tidak lulus jika revisi lewat waktu akibat mahasiswa tidak melakukan revisi
            $schedules = scheduleModel::whereHas('revisions', function($query){
                            $query->whereIn('status',['sedang dikerjakan']);
                          })
            ...
            foreach ($schedules as $schedule) {
                ...
                {
                    if( Carbon::parse($schedule->date)->addDays( (int)$schedule->durasi_revisi ) < now() )
                    {
                        $schedule->sidang->update(['status' => 'tidak lulus']);
                        $schedule->update(['keputusan' => 'tidak lulus']);
                        StatusLogController::addStatusLog($schedule->sidang->id,"-","tidak lulus","tidak lulus by sistem");
                    ...
                    if( Carbon::parse($schedule->date)->addDays( (int)$schedule->durasi_revisi +14 ) < now() )
                    {
                        $schedule->sidang->update(['status' => 'tidak lulus']);
                        $schedule->update(['keputusan' => 'tidak lulus']);
                        StatusLogController::addStatusLog($schedule->sidang->id,"-","tidak lulus","tidak lulus by sistem");
           ...
           Log::error("Error tidakLulusByMahasiswa: ".$e);
        }
    }
    ...
    public function notifToAllAdmin($schedule)
    {
        ...
        $message = "Mahasiswa dengan nim ".$schedule->sidang->mahasiswa->nim." dinyatakan tidak lulus karena tidak menyelesaikan revisi sesuai waktu yang sudah ditentukan";
        $url = "/sidangs";
