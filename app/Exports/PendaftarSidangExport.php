...
class PendaftarSidangExport implements FromView
{
    ...
    public function view(): View
    {
        if ($this->period_id == null)
            $sidangs = Sidang::whereBetween('created_at', [$this->tanggal_awal, $this->tanggal_akhir])->get();
        else
            $sidangs = Sidang::where('period_id', $this->period_id)->get();

        $data = $this->getExportData($sidangs);
        return view('exports.pendaftaran')->with([
            'sidangs' => $sidangs,
            'extensions' => $data
        ]);
    }
